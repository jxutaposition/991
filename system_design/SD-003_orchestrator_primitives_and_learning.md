...
`POST /api/execute` supports two modes:

- `mode: "orchestrated"` (default) — creates a `master_orchestrator` node AND runs a separate LLM planner step (`plan_execution()` in `planner.rs`) that generates preview child nodes. The session starts in `planning` status while the background planner runs. Once complete, preview nodes appear on the canvas and the session moves to `awaiting_approval`. After approval, the master orchestrator runs and spawns child agents (which may differ from the preview).
- `mode: "planned"` — uses a one-shot DAG planner to decompose into a static set of nodes (no master orchestrator).

> **Note:** The original design stated "there is no separate planner step." In practice, the planner runs as a background task during session creation to generate a preview plan, giving the user something to review before approval. The master orchestrator then runs its own decomposition at execution time, potentially diverging from the preview.

---

## Part 2: Primitives

The system has two primitives: skills and tools. In the original design, agents were described as pure runtime compositions. In practice, agents (`agent_definitions`) remain the primary runtime unit with fixed prompts, tools, and judge configs. Skills provide an overlay resolution layer — scoped lessons and context that augment an agent's prompt at runtime.
...
overlays (
  ...
  supersedes UUID,                 -- FK to prior overlay this replaces
  created_at TIMESTAMPTZ,
  updated_at TIMESTAMPTZ
)
```

> **Note:** The original design included a `content_embedding VECTOR(1536)` column for semantic similarity in pattern promotion. This was not implemented — the pattern promoter uses a simpler approach without per-overlay embeddings.
...
## Parts 7-10: Knowledge and Data Architecture

> **Moved to [SD-006: Knowledge and Data Architecture](SD-006_knowledge_and_data_architecture.md).**
>
> SD-006 is the canonical reference for the expert corpus, ingestion pipeline, hybrid retrieval (vector + BM25 + Claude reranking), contextual embeddings, multi-channel learning feeders (Corpus Analyzer, Observation Distiller, Execution Reviewer, Transcript Analyzer), and the knowledge UI. It supersedes the content previously in these sections.
