...
interface Integration {
  ...
  oauth_connect_help?: string;
  oauth_unconfigured_help?: string;
...
export default function IntegrationsPage() {
  const { activeClient, apiFetch, loading: authLoading, token, refreshClients } = useAuth();
    ...
    const fromUrl =
      typeof window !== "undefined"
        ? (new URLSearchParams(window.location.search).get("highlight") ?? "")
            ...
            .filter(Boolean)
        : [];
    const fromStore = readSuggestedIntegrationsFromStorage();
    setHighlightSlugs([...new Set([...fromUrl, ...fromStore])]);
  }, [activeClient]);
  ...
  useEffect(() => {
    ...
    apiFetch(`/api/projects?client_slug=${activeClient}`)
      ...
      .catch(() => {});
  }, [activeClient, apiFetch, authLoading, token]);
  ...
  // Deep link /integrations#notion — search text filters cards out; clear filter and scroll after paint
  useEffect(() => {
    ...
    const applyHash = () => {
      ...
      if (!integrations.some((i) => i.slug === slug)) return;
      setSearchQuery("");
      scrollToSlug(slug);
    };
    applyHash();
  ...
  const refreshCredentials = useCallback(() => {
    if (!token || !activeClient) return;
    const url = activeProject
      ? `/api/projects/${activeProject}/credentials`
      : `/api/clients/${activeClient}/credentials`;
    apiFetch(url)
      .then(async (r) => {
        // Stale active client slug (localStorage) or deleted project → backend NOT_FOUND
        if (r.status === 404) {
          if (activeProject) {
        ...
        return r.json() as Promise<{ credentials?: ConnectedCredential[] }>;
      })
      .then((data) => {
        if (data === null) return;
  ...
  }, [activeClient, activeProject, apiFetch, token, refreshClients]);

  // Only reset + fetch when auth and client are ready. Otherwise refreshCredentials() returns early
  // but this effect used to clear connected first — wiping the map without a refetch (connections "lost").
  useEffect(() => {
    if (authLoading || !token) return;
    if (!activeClient) {
    ...
    setCredScopes({});
    refreshCredentials();
  }, [authLoading, token, activeClient, activeProject, refreshCredentials]);
    ...
    return (
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="border border-amber-200 bg-amber-50 rounded-lg p-4 mt-4">
          <p className="text-sm text-amber-800 font-medium">No client selected</p>
          <p className="text-sm text-amber-700 mt-1">
            Go to{" "}
            <Link href="/settings" className="text-brand hover:underline font-medium">
              Settings
            </Link>{" "}
            to create or select a client before managing integrations.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div>
          ...
          <p className="text-ink-2 text-sm mt-1">
            ...
            {activeProject ? (
              ...
              <> for <span className="font-medium">{activeClient}</span> (client defaults)</>
            )}
            .
          </p>
        </div>
      ...
      {highlightSlugs.length > 0 && (
        <div className="border border-violet-200 bg-violet-50 dark:bg-violet-950/30 dark:border-violet-800 rounded-lg p-3 mb-5 text-sm text-violet-950 dark:text-violet-100">
          <span className="font-medium">Suggested from your SOW:</span>{" "}
          <span className="font-mono text-xs">
            {highlightSlugs.map((slug, i) => (
              <span key={slug}>
                {i > 0 ? ", " : null}
                <button
                  type="button"
          ...
          <span className="block text-xs text-violet-800 dark:text-violet-200/90 mt-1">
            Click a name to jump to it, or clear the search box if cards disappeared. Connect the ones your plan needs before activating on Execute.
          </span>
        </div>
      )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {connectedList.map((integration) => (
                  <IntegrationCard
                    ...
                    sowSuggested={highlightSlugs.includes(integration.slug)}
            ...
            <div className="text-center py-8 space-y-3">
              <p className="text-ink-3 text-sm">
                ...
                {/^https?:\/\//i.test(searchQuery.trim()) ? (
                  <span className="block text-xs mt-2 text-ink-3">
                    That looks like a URL. The search only filters by integration name — clear it to see all cards again.
                  </span>
                ) : null}
              </p>
              <button
...
/** Where execution/run updates are posted for this client — stored as clients.slack_channel_id. */
function SlackRunNotificationsPanel({
  activeClient,
  activeProject,
  apiFetch,
}: {
  ...
  useEffect(() => {
    ...
    apiFetch(`/api/clients/${activeClient}`)
      .then((r) => r.json())
      .then((data) => {
        setChannelId((data.client?.slack_channel_id as string) || "");
        setLoaded(true);
      })
  ...
  const save = useCallback(async () => {
    ...
    try {
      await apiFetch(`/api/clients/${activeClient}`, {
        method: "PATCH",
        body: JSON.stringify({ slack_channel_id: channelId.trim() || "" }),
      });
      setSavedPulse(true);
    ...
    setSaving(false);
  }, [activeClient, activeProject, apiFetch, channelId]);

  if (activeProject) {
    return (
      <div className="mt-3 pt-3 border-t border-rim/80 mx-4 mb-4">
        <p className="text-[11px] leading-snug text-ink-3">
          <span className="font-medium text-ink-2">Run notifications</span> are configured once per client. Select{" "}
          <span className="font-medium text-ink-2">Client defaults</span> above to choose the Slack channel or DM where
          execution updates are posted.
        </p>
      </div>
    );
  }
  ...
  if (!loaded) {
    return (
      <div className="mt-3 pt-3 border-t border-rim/80 mx-4 mb-4">
        <p className="text-[11px] text-ink-3">Loading notification settings…</p>
      </div>
    );
  }
  ...
  return (
    <div className="mt-3 pt-3 border-t border-rim/80 mx-4 mb-4 space-y-2">
      <p className="text-[11px] font-semibold text-ink-2 uppercase tracking-wider">Run notifications</p>
      <p className="text-[11px] leading-snug text-ink-3">
        While executions run for this <span className="font-medium text-ink-2">client</span>, we post status updates to one Slack
        conversation. Paste its ID: <span className="font-mono text-ink-2">C…</span> (public channel),{" "}
        <span className="font-mono text-ink-2">G…</span> (private channel — invite the app first), or{" "}
        <span className="font-mono text-ink-2">D…</span> (DM with the app). Leave empty to disable Slack run updates.
      </p>
      <div className="flex gap-2">
        <div className="relative flex-1">
          <Hash className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-ink-3 pointer-events-none" />
          <input
            type="text"
            value={channelId}
            onChange={(e) => setChannelId(e.target.value)}
            placeholder="e.g. C01ABCDEF or D09ABCDEF"
            className="w-full bg-surface border border-rim rounded-lg px-3 py-1.5 pl-8 text-xs font-mono text-ink focus:outline-none focus:border-brand"
          />
        </div>
        <button
          type="button"
          onClick={save}
          disabled={saving}
          className="shrink-0 bg-brand text-white px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-brand-hover disabled:opacity-50"
        >
          {savedPulse ? "Saved" : saving ? "Saving…" : "Save"}
        </button>
      </div>
      <p className="text-[10px] leading-snug text-ink-3">
        In Slack: open the channel or DM → title → copy <span className="font-medium">Channel ID</span> at the bottom. For a DM,
        message the app first so the conversation exists.
      </p>
    </div>
  );
}
  ...
  return (
    <div
      ...
      className={`rounded-xl border transition-all ${
        sowSuggested ? "ring-2 ring-violet-400/70 ring-offset-2 ring-offset-surface " : ""
      }${
        isConnected
    ...
    >
      ...
      <div className="flex items-center gap-3 p-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          {integration.oauth_connect_help && isOAuth && hasOAuthConfig && !isConnected && (
            <p className="text-[11px] text-ink-2 mt-1 leading-snug">{integration.oauth_connect_help}</p>
          )}
        </div>
      ...
      {isExpanded && !isConnected && (
        <div className="px-4 pb-4 pt-0 border-t border-rim">
          <div className="pt-3 space-y-2">
            {isOAuth && !hasOAuthConfig && integration.oauth_unconfigured_help && (
              <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5 text-[11px] leading-snug text-amber-950">
                {integration.oauth_unconfigured_help}
              </div>
            )}
