...
Before building any workflow, verify you have all the runtime configuration you need.
...
1. Verify n8n credentials are configured (API key + base URL)
2. For workflows that post to Slack: check that the Slack credential exists in n8n's credential store (`GET /api/v1/credentials`). If missing, use `request_user_action` with steps to guide the user through adding it in the n8n UI
3. For workflows that post to Slack: ask the user which channel to post to using `request_user_action` with a `type: "inputs"` section and `input_type: "slack_channel"`
4. For workflows that create Notion pages: ask which database to use with `input_type: "notion_database"`
5. For webhook-triggered workflows: the webhook URL is generated by n8n — pass it to the upstream system, don't ask the user for it

**If you need information the user hasn't provided:**
1. Check if it was provided in the task context or upstream outputs
2. If not, use `request_user_action` with a `type: "inputs"` section to ask
3. Specify the `input_type` so the UI shows the right picker
4. Wait for the user's reply before proceeding
5. Validate the provided value

Do NOT guess or hardcode integration targets. Do NOT create new resources (channels, databases) without asking first.
